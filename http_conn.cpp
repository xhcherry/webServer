...
// 网站的根目录
const char* doc_root = "/root/webserver/webServer/resources";
...
http_conn::HTTP_CODE http_conn::parse_request_line(char* text) {
    ...
    *m_version++ = '\0';
    ...
    // http://172.17.8.105:9999/index.html
    if (strncasecmp(m_url, "http://", 7) == 0) {
        m_url += 7;                  // http://172.17.8.105:9999/index.html
        m_url = strchr(m_url, '/');  // index.html
    }
    if (!m_url || m_url[0] != '/') {
        return BAD_REQUEST;
    }
    m_check_state = CHECK_STATE_HEADER;  // 主检测机检查状态变成检查请求头
    return NO_REQUEST;
}
// 解析HTTP请求的一个头部文件
    ...
    for (; m_checked_index < m_read_idx; ++m_checked_index) {
        ...
        if (temp == '\r') {
            ...
            if ((m_checked_index > 1) &&
                (m_read_buf[m_checked_index - 1] == '\r')) {
...
http_conn::HTTP_CODE http_conn::do_request() {
    // "/home/webserver/webServer/resources"
